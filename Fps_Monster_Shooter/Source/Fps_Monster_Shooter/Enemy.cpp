...
AEnemy::AEnemy()
{
	...
	//How long for ai to forget the player
	SightConfig->SetMaxAge(0.1f);

	AIPerComp->ConfigureSense(*SightConfig);

	//Setting the SightConfig as the dominant senese
...
void AEnemy::OnSensed(const TArray<AActor*>& UpdatedActors)
{
	...
	for (int i = 0; i < UpdatedActors.Num(); i++)
	{
		...
		//Subtracting location of player from the enemy
		if (Info.LastSensedStimuli[0].WasSuccessfullySensed())
		{
			FVector dir = UpdatedActors[i]->GetActorLocation() - GetActorLocation();
			dir.Z = 0.0f;

			//GetSafeNormal gets length of vector and checks if it is safe to do so
		...
		{
			//if not sensed player, go back to baselocation
			FVector dir = BaseLocation - GetActorLocation();
			dir.Z = 0.0f;
			//if away from baselocation, set rotation and velocity to travel back from current position
...
//Rotate Actor by subtracting player and enemy position
